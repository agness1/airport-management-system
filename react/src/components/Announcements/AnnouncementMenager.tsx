...
const AnnouncementMenager: FC = () => {
    ...
    const announcementList = () => {
        if (data !== undefined && data !== null) {
            const announcement = data.map((item: any) => {
                return (
                    <div className="flex flex-col w-11/12 items-center gap-4 bg-white rounded-md">
                        <p className="text-xl font-medium bg-black w-full text-white text-center p-2 rounded-t-md">
                            {item.type}
                        </p>
                        <p className="text-xl font-medium">{item.title}</p>
                        <p className="text-center break-all p-4">
                            {item.description}
                        </p>
                        <button
                            className="bg-blue p-2 mb-4 text-white text-xl font-medium lg:w-1/3 m-auto rounded-md hover:bg-green transition-all"
                            onClick={() =>
                                deleteApi(
